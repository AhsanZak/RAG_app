...
import { llmModelsAPI, excelChatAPI, embeddingModelsAPI } from '../services/api';
...
const ExcelChat = ({ onBack }) => {
  ...
  const [sessionReady, setSessionReady] = useState(false); // Track if session has processed Excel files
  ...
  const loadSessions = async () => {
    try {
      const sessionsData = await excelChatAPI.getSessions();
      // Filter to only show Excel sessions
      const excelSessions = sessionsData.filter(s => s.session_type === 'excel');
      const formattedSessions = excelSessions.map(s => ({
        id: s.id,
        name: s.name,
  ...
  const createNewSession = async () => {
    // Create a placeholder session (will be replaced when Excel files are processed)
    const newSession = {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      ...
      const response = await excelChatAPI.uploadExcel(file);
      ...
      message.success(`Excel file "${file.name}" uploaded successfully`);
    } catch (error) {
  ...
  const handleProcessFiles = async () => {
    if (files.length === 0) {
      message.warning('Please upload Excel files first');
      return;
    }
    ...
    try {
      ...
      const sessionName = files.length === 1 
        ? files[0].name.replace(/\.(xlsx|xls|xlsm)$/i, '') 
        : `${files.length} Excel Files`;
      const response = await excelChatAPI.processExcels(
        filesData, 
        sessionName, 
        selectedModel, 
        null,
      ...
      Modal.success({
        title: 'Excel Files Processed Successfully',
        content: `${response.total_chunks} data chunks have been vectorized and stored in ChromaDB. You can now start chatting!`,
      });
      ...
      Modal.error({
        ...
        content: error.response?.data?.detail || 'Failed to process Excel files. Please try again.',
      });
    } finally {
  ...
  const handleSelectSession = async (session) => {
    ...
    if (session.id && session.hasVectorizedData) {
      ...
      try {
        const messages = await excelChatAPI.getSessionMessages(session.id);
  ...
  const handleSendMessage = async () => {
    ...
    if (!sessionReady) {
      message.warning('Please process Excel files first before chatting');
      return;
    }

    if (!currentSession.id) {
      message.warning('Session not ready. Please process Excel files first.');
      return;
    }
    ...
    try {
      ...
      const response = await excelChatAPI.chat(
        currentSession.id,
        messageText,
        selectedModel
      );
    ...
    <Layout className="pdf-chat-layout">
      <Header className="pdf-chat-header">
        <Space>
          ...
          <Title level={4} style={{ margin: 0 }}>Excel Chat</Title>
        </Space>
        <Space>
        ...
        <div className="pdf-chat-container">
          ...
          <div className="pdf-chat-sidebar">
            ...
            >
              {!sessionReady && (
                <Alert
                  message="Upload Excel files and process them to enable chat"
                  description="Excel files will be vectorized and stored in ChromaDB for RAG-based chat"
                  type="info"
                  showIcon
              ...
              <Dragger {...uploadProps} disabled={processingFiles}>
                ...
                <p className="ant-upload-text" style={{ marginBottom: 4 }}>Click or drag Excel files to upload</p>
                <p className="ant-upload-hint" style={{ marginBottom: 0 }}>Single or multiple files (.xlsx, .xls, .xlsm)</p>
              </Dragger>
              ...
              {files.length > 0 && (
                <div style={{ marginTop: 12 }}>
                  ...
                  <Button
                    ...
                    style={{ marginTop: 16 }}
                  >
                    {processingFiles ? 'Processing & Vectorizing...' : 'Process & Vectorize Excel Files'}
                  </Button>
                  {!selectedModel && (
                    <Alert
              ...
              {sessionReady && (
                <Alert
                  ...
                  description="Excel files have been processed and vectorized. You can now chat!"
                  type="success"
                  showIcon
                  style={{ marginTop: 12 }}
                />
                      ...
                      <List.Item.Meta
                        ...
                        description={
                          <span style={{ fontSize: 12 }}>
                            {session.hasVectorizedData ? `${session.fileCount} file(s) - Ready` : 'New session - Upload Excel Files'}
                          </span>
                        }
                      />
          ...
          <div className="pdf-chat-main">
            {!currentSession ? (
              <div className="empty-chat-state">
                <FileExcelOutlined style={{ fontSize: '64px', color: '#d9d9d9', marginBottom: 16 }} />
                <Title level={3}>Ready to Start</Title>
                <Paragraph>
                  Upload Excel files on the left side, then process them to enable chat.
                </Paragraph>
              </div>
            ) : (
              <>
                <div className="chat-messages-container">
                  {chatMessages.length === 0 ? (
                    <div className="empty-messages-state">
                      ...
                      <Title level={4}>
                        {sessionReady ? 'Start Chatting' : 'Process Excel Files First'}
                      </Title>
                      <Paragraph>
                        {sessionReady ? (
                          <>
                            Ask questions about your Excel data. The AI will use the vectorized content
                            from ChromaDB to provide accurate answers.
                          </>
                        ) : (
                          <>
                            Upload Excel files on the left side and click "Process & Vectorize Excel Files" 
                            to enable chat. Excel data will be converted to vectors and stored in ChromaDB.
                          </>
                        )}
                      </Paragraph>
                    </div>
                ...
                <Footer className="chat-input-footer">
                    ...
                    <Input.TextArea
                      ...
                      placeholder={
                        sessionReady 
                          ...
                          : "Please process Excel files first to enable chat..."
                      }
                  ...
                  {!sessionReady && (
                    <Alert
                      ...
                      description="Please upload and process Excel files first to enable chat"
                      type="warning"
                      showIcon
                      style={{ marginTop: 8 }}
                    />
